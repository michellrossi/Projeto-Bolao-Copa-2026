...
export function NavBar() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-dark/90 backdrop-blur-2xl border-t border-white/5 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] flex justify-around items-center h-20 px-2 pb-safe">
      {links.map((link) => (
        <NavLink 
          key={link.name} 
          to={link.path}
